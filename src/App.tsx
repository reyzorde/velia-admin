...
function Centers() {
  ...
  const load = useCallback(async () => {
    const [{ data }, { data: p }] = await Promise.all([
      supabase.from('centers').select('id, name, owner_id, created_at, subscriptions(id, plan_id, status, expires_at)').order('created_at', { ascending: false }),
  ...
  const changePlan = async (centerId: string, planId: string) => {
    setMsg('');
    const sub = rows.find((r) => r.id === centerId)?.subscriptions;
    const subRow = Array.isArray(sub) ? sub[0] : sub;
    if (subRow?.id) {
      const { error } = await supabase.from('subscriptions').update({ plan_id: planId, status: 'active', updated_at: new Date().toISOString() }).eq('id', subRow.id);
      setMsg(error ? error.message : 'Tarif yangilandi');
    } else {
      const { error } = await supabase.from('subscriptions').insert({ center_id: centerId, plan_id: planId, status: 'active', billing_period: 'monthly' });
          ...
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr key={r.id}>
                  <td>{r.name}</td>
                  <td><code>{sub?.plan_id || '—'}</code></td>
                  <td>{sub?.status || '—'}</td>
                  <td>
                    <select className="input" style={{ maxWidth: 180 }} defaultValue={sub?.plan_id || ''} onChange={(e) => { if (e.target.value) void changePlan(r.id, e.target.value); }}>
...
function Subscriptions() {
  const [rows, setRows] = useState<any[]>([]);
  const load = useCallback(async () => {
    const { data } = await supabase.from('subscriptions').select('id, plan_id, status, billing_period, expires_at, centers(name)').order('updated_at', { ascending: false }).limit(200);
    setRows(data || []);
  }, []);
  useEffect(() => { void load(); }, [load]);
          ...
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr key={r.id}>
                  <td>{c?.name || '—'}</td>
                  <td><code>{r.plan_id}</code></td>
                  <td>{r.status}</td>
                  <td style={{ display: 'flex', gap: 6 }}>
                    <button className="btn ok" type="button" onClick={() => void setStatus(r.id, 'active')}>Active</button>
                    <button className="btn danger" type="button" onClick={() => void setStatus(r.id, 'inactive')}>Inactive</button>
                  </td>
                </tr>
              );
            })}
...
function UsersPage() {
  ...
  const [msg, setMsg] = useState('');
  const load = useCallback(async () => {
    const { data } = await supabase.from('profiles').select('id, full_name, email, is_platform_admin, created_at').order('created_at', { ascending: false }).limit(300);
    setRows(data || []);
  }, []);
  useEffect(() => { void load(); }, [load]);
  ...
  return (
    <div>
      ...
      <p className="muted">Asosiy admin Supabase Auth da yaratiladi. Boshqalar: user royxatdan otadi, keyin bu yerda platform_admin yoqiladi.</p>
      {msg && <p className="muted">{msg}</p>}
      <div className="card table-wrap">
        <table>
